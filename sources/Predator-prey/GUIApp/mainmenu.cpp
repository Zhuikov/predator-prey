...
MainMenu::MainMenu(QWidget* parent, Settings* settings) : QWidget(parent, Qt::WindowTitleHint)
{
    ...
    new_model_button = new QPushButton("Create new model", this);
    new_model_button->setStyleSheet(button_style);
    new_model_button->resize(BUTTON_SIZE);
    new_model_button->move(WINDOW_SIZE.width() - 205,
    ...
    settings_button->move(WINDOW_SIZE.width() - 205,
    ...
    exit_button->move(WINDOW_SIZE.width() - 205,
